...
`@peter.naydenov/visual-controller-for-svelte5`. Each record covers one
version transition: what changed, step-by-step migration, and code
comparisons.
...
### Step 4 — Update component props to receive `dependencies` and `setupUpdates`

In v2 the component received `dependencies` as a prop. In v3 the controller also passes `setupUpdates` as a top-level prop, so the component can pick it up directly:

```svelte
<!-- v3 -->
<script>
  let { greeting = 'Hi', dependencies, setupUpdates } = $props()

  let message = $state(greeting)

  setupUpdates({
    changeMessage (newMsg) { message = newMsg }
  })
</script>
```

If you were already reading `setupUpdates` from `dependencies` in v2, switch to the dedicated prop — the controller wires both in v3.
...
html.publish(MyComponent, {}, 'header')   // detects non-empty → hydrate
...
The auto-detection (empty range → fresh mount, non-empty → mount on existing DOM) is preserved.


### Pattern E — Destroy every app at once (new in v3)
...
### Pattern F — External control via `setupUpdates`
...
- [ ] Update Svelte components to read `setupUpdates` (and `dependencies`) as `$props()`.
...
- [ ] Run your tests. The Svelte component code itself should not need changes beyond the props destructure.
